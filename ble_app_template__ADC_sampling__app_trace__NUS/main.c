 ...
 * but is lid up when the board is connected.
 *
...
#define ADC_SAMPLING_INTERVAL            APP_TIMER_TICKS(200, APP_TIMER_PRESCALER) /**< Sampling rate for the ADC */
 ...
 *           it to the app_trace module (UART).
 */
/**@snippet [Handling the data received over BLE] */
void nus_data_handler(ble_nus_t * p_nus, uint8_t * p_data, uint16_t length)
{
		app_trace_log("NUS service: ");
    for (int i = 0; i < length; i++)
    {
        app_trace_log("%c",p_data[i]);
    }
    app_trace_log("\r\n");
}
...
/*
 * Print 3xLSB from the ADC result on LED_2, LED_3 and LED_4 on PCA10028
 */
void write_adc_result_to_leds(uint16_t adc_result)
{
	if(adc_result & 0x01)  
	{
